...
class Animatable:
    def __init__(self,
        ...
        self.current_time = 0
        self.current_image = self.animations_dict[self.current_animation][self.current_frame][0]
...
class Entity(Movable, Animatable):
    def __init__(self,
        ...
        hitbox_width = self.current_image.width - self.current_image.width // 4
        hitbox_height = self.current_image.height // 8

        hitbox_x = self.pos.x + (self.current_image.width - hitbox_width) // 2
        hitbox_y = self.pos.y + self.current_image.height - hitbox_height
    ...
    def update(self, alter_pos: tuple[int, int] | pg.Vector2 | None = None) -> None:
        ...
        hitbox_x = pos.x + (self.current_image.width - hitbox_width) // 2
        hitbox_y = pos.y + self.current_image.height - hitbox_height
